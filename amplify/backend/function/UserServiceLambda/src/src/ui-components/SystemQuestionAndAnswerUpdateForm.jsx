...
export default function SystemQuestionAndAnswerUpdateForm(props) {
  const {
    id,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...systemQuestionAndAnswerRecord };
    setAnswerVariations(cleanValues.answerVariations ?? []);
    setCurrentAnswerVariationsValue(undefined);
    setQuestionVariations(cleanValues.questionVariations ?? []);
    setCurrentQuestionVariationsValue(undefined);
    setErrors({});
  };
  const [systemQuestionAndAnswerRecord, setSystemQuestionAndAnswerRecord] =
    React.useState(systemQuestionAndAnswer);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(SystemQuestionAndAnswer, id)
        : systemQuestionAndAnswer;
      setSystemQuestionAndAnswerRecord(record);
    };
    queryData();
  }, [id, systemQuestionAndAnswer]);
  React.useEffect(resetStateValues, [systemQuestionAndAnswerRecord]);
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            SystemQuestionAndAnswer.copyOf(
              systemQuestionAndAnswerRecord,
              (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "SystemQuestionAndAnswerUpdateForm")}
    >
      <ArrayField
