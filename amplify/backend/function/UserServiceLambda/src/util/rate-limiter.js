...
class RateLimiter {
    ...
    refillTokens(identifier) {
      const now = Date.now();
      let { tokenPerMin, tokenCapacity, lastRefillTime, availableTokens } = dynamo.getItem(`${identifier}`, process.env.API_MYAPPLICATIONSECRETARYAMPLIFY_RATELIMITTABLE_NAME) ?? {};
